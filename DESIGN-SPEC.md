...
- 动作条：`主工作台`、`LLM配置`、`退出` 三个按钮横向单行排列，与触发器垂直居中并保持同一水平线
- 触发器与动作条间距：8–10px
- 动作条容器：`background: var(--color-surface)`，`border: 1px solid var(--color-border)`，`border-radius: var(--radius-md)`，`padding: 6px`，`box-shadow: var(--shadow-whisper)`
- 菜单项：非胶囊，最小宽度 96px，圆角 `var(--radius-md)`，`border: 1px solid var(--color-border)`，使用轻量 hover / active 背景变化
...
- 用途：只用于用户操作结果反馈，例如保存成功、上传失败、创建面试成功、请求异常
